...
public class NetCacheUtils {
    ...
    class BitmapTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap result) {
            if(result != null){
                ...
                if(url.equals(bindUrl)){    // 确保图片设定给了正确的imageView,避免imageView重用导致程序报错
                    ivPic.setImageBitmap(result);
                    mLocalCacheUtils.setBitmapToLocal(url,result);  // 将图片保存在本地
                    mMemoryCacheUtils.setBitmapToMemory(url,result);    // 将图片保存到内存中
    ...
    private Bitmap downloadBitmap(String url) {
        ...
        try {
            ...
            int responseCode = conn.getResponseCode();  // 拿到响应码
            if (responseCode == 200) {
                InputStream inputStream = conn.getInputStream();

                // 图片压缩处理
                BitmapFactory.Options option = new BitmapFactory.Options();
                option.inSampleSize = 2;    // 宽高都压缩为原来的1/2,此参数需要根据图片要展示的大小来确定
